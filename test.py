...
def add_sample_data():
    ...
    if conn:
        ...
        if cursor.fetchone()[0] == 0:
            ...
            movie_id = cursor.fetchone()[0]
            sessions = [
                (movie_id, "2023-10-26 19:00"),
                (movie_id, "2023-10-26 22:00")
            ]
            cursor.executemany("INSERT INTO sessions (movie_id, show_time) VALUES (?, ?)", sessions)
            cursor.execute("SELECT id FROM movies WHERE title='Бойцовский клуб'")
            movie_id = cursor.fetchone()[0]
            sessions = [
                (movie_id, "2023-10-27 20:00"),
                (movie_id, "2023-10-27 23:00")
            ]
            cursor.executemany("INSERT INTO sessions (movie_id, show_time) VALUES (?, ?)", sessions)
            conn.commit()
        conn.close()
...
def view_tickets():
    ...
    if conn:
        cursor = conn.cursor()
        cursor.execute("SELECT t.id, m.title, s.show_time, t.seat_number, t.customer_name, t.order_number, t.status FROM tickets t JOIN sessions s ON t.session_id = s.id JOIN movies m ON s.movie_id = m.id")
